...
3. **`Environment.hostId` is immutable** — set at creation, never updated
   ...
   This only blocks reassigning an environment to a genuinely *different*
   logical host — a vanished host reclaiming its **own** identity on fresh
   compute doesn't need it: a persisted `auth.json` reconnects through the
   existing `upsertHost` path with no `Environment.hostId` mutation,
   because the host row's identity never actually changed (confirmed live,
   C6/P6 in [design-position-and-probes.md](design-position-and-probes.md)).
...
unless compute recreation is intended as a continuation of the *same*
logical host, in which case C6/P6 in
[design-position-and-probes.md](design-position-and-probes.md) already
covers it: a persisted daemon identity reconnecting on fresh compute (via a
surviving `auth.json`, or a reissued credential when even that's lost)
means `AgentRuntime.resumeThread`'s same-host assumption keeps holding, so
nothing new is needed for the disposable-container case this section
worries about most. This doesn't help the genuinely different case — moving
a thread to a *different* host — which still has no mechanism.
